...
def verbose_print(verbose, *args):
    """prints args if verbose is true"""
    if verbose:
        print(*args)
...
def build_mechanism(modeldir, mechanism, home, arch, verbose=True, cleanup=True):
    ...
    vprint("building mechanisms in directory", mechdir)
    os.chdir(mechdir)
    vprint("running command", cleancmd, "in directory", mechdir)
    # Erases any old compiled files
    if cleanup:
        subprocess.call(cleancmd)
    vprint("running command", buildcmd, "in directory", mechdir)
...
def build(NRNHOME, NRNARCH, paramdir=".", verbose=True, cleanup=False):
    ...
    for mechanism in MECHANISMS:
        build_mechanism(os.getcwd(), mechanism, NRNHOME, NRNARCH, verbose, cleanup)


def load_mechanism(modeldir, mechanism, arch, verbose=True):
    vprint = lambda *a: verbose_print(verbose, *a)

    """Runs the commands necessary to load an NMODL mechanism"""
    mechdir = os.path.join(modeldir, "mechanisms", mechanism)
    os.chdir(mechdir)
    vprint("loading mechanisms in directory", mechdir)
    # Loads the resulting shared library
    if sys.platform.startswith("win"):
        h.nrn_load_dll(
            os.path.normpath(os.path.join(mechdir, arch, "libs", "libnrnmech.dll"))
        )
    else:
        h.nrn_load_dll(
            os.path.normpath(os.path.join(mechdir, arch, ".libs", "libnrnmech.so"))
        )
    os.chdir(modeldir)


def init_populations(
    NRNHOME, NRNARCH, verbose=True, paramdir=".", iterparam=None, save_populations=False
):
    vprint = lambda *a: verbose_print(verbose, *a)
    ...
    if iterparam:
        vprint("Using iterparam:%s", iterparam)
    ...
    if iterparam:
        ...
        vprint(
            "Using cell ids %s from file %s row %d"
            % (iter_name, iter_path, iter_index)
        )
    ...
    h(
        'pathactiveMfibres1="'
        + (os.path.normpath(paramdir + "/activeMfibres1.dat"))
        + '"'
    )
    ...
    vprint("Loading object declarations")
    h.xopen("objects.hoc")
    ...
    vprint("Loading Granule cell population")
    h.xopen(os.path.normpath(os.getcwd() + "/populations/GranulePopulation.hoc"))

    # Create the mossy fiber input source
    vprint("Loading Mossy fiber template")
    h.xopen(os.path.normpath(os.getcwd() + "/templates/MF_template.hoc"))
    vprint("Creating Mossy fiber population")
    h.xopen(os.path.normpath(os.getcwd() + "/populations/MFPopulation.hoc"))

    # Create the MLI population
    # vprint("Loading MLI template")
    # h.xopen(os.path.normpath(os.getcwd() + "/templates/MLI_template.hoc"))
    # vprint("Creating MLI population")
    # h.xopen(os.path.normpath(os.getcwd() + "/populations/MLIPopulation.hoc"))
    ...
    if h.MLplug == 1:  # This part is reserved for future development
        ...
        if verbose:
            print("Cell populations created")
...
def init_connections(NRNHOME, NRNARCH, verbose=True):
    ...
    vprint = lambda *a: verbose_print(verbose, *a)
    vprint("Creating Network Connections")
    ...
    if (h.MF_GC_con > 0) | (h.PF_GoC_con > 0) | (h.GoC_GoC_inh_con > 0):
        ...
        h(
            'pathMFtoGCsources="'
            + (os.path.normpath("MFtoGCsources%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathMFtoGCtargets="'
            + (os.path.normpath("MFtoGCtargets%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathMFtoGCdistances="'
            + (os.path.normpath("MFtoGCdistances%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathPFtoGoCsources="'
            + (os.path.normpath("PFtoGoCsources%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathPFtoGoCtargets="'
            + (os.path.normpath("PFtoGoCtargets%d.dat" % int(pc.id())))
            + '"'
        )

        h("strdef pathPFtoGoCdistances")
        h(
            'pathPFtoGoCdistances="'
            + (os.path.normpath("PFtoGoCdistances%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathPFtoGoCsegments="'
            + (os.path.normpath("PFtoGoCsegments%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathAAtoGoCsources="'
            + (os.path.normpath("AAtoGoCsources%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathAAtoGoCtargets="'
            + (os.path.normpath("AAtoGoCtargets%d.dat" % int(pc.id())))
            + '"'
        )

        h("strdef pathAAtoGoCdistances")
        h(
            'pathAAtoGoCdistances="'
            + (os.path.normpath("AAtoGoCdistances%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathAAtoGoCsegments="'
            + (os.path.normpath("AAtoGoCsegments%d.dat" % int(pc.id())))
            + '"'
        )
        ...
        h(
            'pathGoCtoGoCgapsources="'
            + (os.path.normpath("GoCtoGoCgapsources.dat"))
            + '"'
        )
        ...
        h(
            'pathGoCtoGoCgaptargets="'
            + (os.path.normpath("GoCtoGoCgaptargets.dat"))
            + '"'
        )
        ...
        h(
            'pathGoCtoGoCgapdistances="'
            + (os.path.normpath("GoCtoGoCgapdistances.dat"))
            + '"'
        )

        if verbose:
            print("Reloading the sorted GC and GoC coordinates")
        ...
        h(
            'pathGoCcoordinates1="'
            + (os.path.normpath("GoCcoordinates.sorted.dat"))
            + '"'
        )

        h(
            'pathAdendcoordinates1="'
            + (os.path.normpath("GoCadendcoordinates.sorted.dat"))
            + '"'
        )

        h(
            'pathBdendcoordinates1="'
            + (os.path.normpath("GoCbdendcoordinates.sorted.dat"))
            + '"'
        )
        ...
        h(
            "GolgiPop.Adendcoordinates.scanf(fileAdendcoordinates1,GolgiPop.numGoC,3*(GoC_nDendML))"
        )
        h(
            "GolgiPop.Bdendcoordinates.scanf(fileBdendcoordinates1,GolgiPop.numGoC,3*(numDendGolgi-GoC_nDendML))"
        )
    ...
    vprint("Loading conectivity rules and synaptic delays")
    h.xopen("enPassage.hoc")

    vprint("Creating MF to GoC")

    h.xopen("netconMFtoGoC.hoc")
    w = h.ncMFtoGoC[0].enP

    vprint("Calling connectPops4")
    ...
    vprint("Creating MF to GC")
    h.xopen("netconMFtoGC.hoc")

    vprint("Creating GoC to GC")
    h.xopen("netconGoCtoGC.hoc")

    vprint("Creating GC to GoC")
    h.xopen("netconGCtoGoC.hoc")

    vprint("Creating GoC to GoC")
    ...
    if h.MLplug == 1:  # This part is reserved for future development
        h.xopen("trial.hoc")
        if verbose:
            print("Creating PF to SC")
        h.xopen("netconPFtoSC.hoc")
        if verbose:
            print("Creating PF to BC")
        h.xopen("netconPFtoBC.hoc")
        if verbose:
            print("Creating SC to PC")
        # h.xopen("netconSCtoPC.hoc")
        if verbose:
            print("Creating BC to PC")
    ...
    vprint("Connectivity patterns created")
...
if __name__ == "__main__":
    ...
    optparser.add_option("--verbose", action="store_true", help="verbose mode")
    optparser.add_option(
        "--cleanup",
        action="store_true",
    ...
    optparser.add_option(
        "--build", action="store_true", help="build mechanisms and exit"
    )
    optparser.add_option(
        "--initpop", action="store_true", help="instantiate the populations and exit"
    )
    optparser.add_option(
        "--init", action="store_true", help="instantiate the network and exit"
    )
    ...
    vprint("NEURON home is", NRNHOME)
    vprint("NEURON arch is", NRNARCH)
    h("objref NCl")
    h("objref RCl")
    h("objref l2")
    h("objref l1")
    ...
    vprint("Loading global parameters from", pathparams)
    h.xopen(pathparams)
    ...
    if options.build:
        ...
        init_populations(
            NRNHOME, NRNARCH, verbose=verbose, paramdir=paramdir, iterparam=iterparam
        )
